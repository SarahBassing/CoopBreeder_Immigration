#-------------------------------------------------------------------------------
  ...
  #  Logistic Regression: Pr(Short-Distance Immigrant) ~ Region
  #  Code by Bassing et al.
#-------------------------------------------------------------------------------
  #  This model tests for the effect of study area on the probability a wolf group 
  #  in central Idaho has a short-distance immigrant in it and estiamtes the mean 
  #  probabilty of a group containing a short-distance immigrant in the east and 
...
#-------------------------------------------------------------------------------
  ...
  #sink("./Idaho_SDI_Region.txt")
  sink("G:/My Drive/1_Repositories/CoopBreeder_Immigration/Idaho_SDI_Region.txt")
  cat("
  model {
  ...
  #  Likelihood
    ...
    #  Testing for effect of study area on Pr(SDI joins a group)
...
#------------------------------------------------------------------------------
  ...
  out <- jags(
    ...
    inits, 
    params, 
    #"./Idaho_SDI_Region.txt", 
    "G:/My Drive/1_Repositories/CoopBreeder_Immigration/Idaho_SDI_Region.txt",
    n.chains=nc, 
    n.thin=nt, 
    n.iter=ni, 
    n.burnin=nb
  ...
  # write.table(out$BUGS$summary, file = "./Output/Idaho_SDI_Region.txt", sep = "\t")
  # Idaho_SDI_Region_output <- out
  # save(Idaho_SDI_Region_output, file = "./Output/Idaho_SDI_Region.RData")
